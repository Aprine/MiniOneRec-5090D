...
SAMPLE="${SAMPLE:-1024}"
EPOCHS="${EPOCHS:-1}"
RUN_ID="${RUN_ID:-baseline_finalonly_freeze_qwen25_3b_industrial}"
OUTPUT_DIR="${OUTPUT_DIR:-output_dir/qwen25_3b_${CATEGORY}_single5090d_finalonly_sft}"
RESULT_DIR="${RESULT_DIR:-results/a0_finalonly_single5090d}"
...
echo "[R0] A0 final-only SFT -> ${OUTPUT_DIR}"
bash repro/run_a0_finalonly_sft_single_5090d.sh 2>&1 | tee "${SFT_LOG}"

echo "[R0] Evaluation -> ${RESULT_DIR}"
